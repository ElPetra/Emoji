...
function filterKey(obj) {
  //создаем функцию, чтобы убрать дубли
  ...
  obj.filter = [...new Set(obj.keywords.split(" "))].join(" ");
}
...
function search(obj, string) {  //создаем функцию поиска с двумя параметрами
  ...
  containerNode.innerHTML = ""; //делаем страницу пустой от всех карточек, чтобы на пустой странице отобразились только отфильтрованные карточки
  return renderCard(find); //возвращаем результат функции renderCard от переменной find
}

inputNode.onkeydown = function (event) {
  //создаем событие при нажатии на какую-то клавишу (onkeypress)
  if (event.key === "Enter") {
    //условие: если клавиша Enter, то ...
